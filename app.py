...
def get_openai_embeddings(texts_tuple, api_key, batch_size=256, _progress_bar_placeholder=None):
    ...
    """
    texts = list(texts_tuple)  # Konwersja z tuple do list
...
def generate_titles(api_key, keyword, volume, competitor_url, max_retries=3):
...
st.title("🚀 Planer Treści SEO [Wersja Hybrydowa v7 - PRO]")
        ...
        for _, row in df_gap.iterrows():
            ...
            if keyword_lower in ranking_map:
                results.append({
                    ...
                    'Status': 'Już istnieje',
                    'Akcja / Dopasowany URL': ranking_map[keyword_lower],
                    'Podobieństwo': 1.00
                })
            else:
